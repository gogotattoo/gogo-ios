...
extension DesignViewModel: Realmable {
  ...
  func syncViewModelToModel() {
    ...
    self.design.videosIPFS.append(objectsIn: (self.videosIPFS).map({ RealmString(value: [$0]) }))
    ...
    self.design.artType = self.artType.rawValue
  ...
  func syncModelToViewModel() {
    ...
    if let blockChain = self.design.blockChain?.copy() as? BlockChain {
      self.blockChain = blockChain
    }
    if let artType = ArtType(rawValue: self.design.artType) {
