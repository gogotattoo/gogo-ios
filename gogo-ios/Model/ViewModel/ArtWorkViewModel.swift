...
extension ArtWorkViewModel: Realmable {
  ...
  func syncViewModelToModel() {
    ...
    self.artWork.videosIPFS.append(objectsIn: (self.videosIPFS).map({ RealmString(value: [$0]) }))
    ...
    self.artWork.blockChain = self.blockChain
    self.artWork.artType = self.artType.rawValue
  ...
  func syncModelToViewModel() {
    ...
    for videoIPFS: RealmString in self.artWork.videosIPFS {
      self.videosIPFS.append(videoIPFS.stringValue)
    }
    if let blockChain = self.artWork.blockChain?.copy() as? BlockChain {
      self.blockChain = blockChain
    }
    if let artType = ArtType(rawValue: self.artWork.artType) {
