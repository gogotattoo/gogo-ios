...
extension PiercingViewModel: Realmable {
  ...
  func syncViewModelToModel() {
    ...
    self.piercing.videosIPFS.append(objectsIn: (self.videosIPFS).map({ RealmString(value: [$0]) }))
    ...
    self.piercing.artType = self.artType.rawValue
  ...
  func syncModelToViewModel() {
    ...
    if let blockChain = self.piercing.blockChain?.copy() as? BlockChain {
      self.blockChain = blockChain
    }
    if let artType = ArtType(rawValue: self.piercing.artType) {
