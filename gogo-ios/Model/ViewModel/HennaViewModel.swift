...
extension HennaViewModel: Realmable {
  ...
  func syncViewModelToModel() {
    ...
    self.henna.videosIPFS.append(objectsIn: (self.videosIPFS).map({ RealmString(value: [$0]) }))
    ...
    self.henna.artType = self.artType.rawValue
  ...
  func syncModelToViewModel() {
    ...
    if let blockChain = self.henna.blockChain?.copy() as? BlockChain {
      self.blockChain = blockChain
    }
    if let artType = ArtType(rawValue: self.henna.artType) {
