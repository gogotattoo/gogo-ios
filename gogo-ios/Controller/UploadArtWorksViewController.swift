...
class UploadArtWorksViewController: BaseViewController {
  ...
  func basicUI() {
    ...
    self.artWorkTypeTextField.text = MainManager.sharedInstance.uploadArtType.rawValue
  ...
  @objc func uploadAction(_ sender: UIBarButtonItem) {
    // TODO: optimize
  ...
  func requestIfUploadImageSucceed(dictionary: Dictionary<String, String>) {
    ...
    let path = "/\(MainManager.sharedInstance.uploadArtistName)/\(MainManager.sharedInstance.uploadArtType.rawValue)/\(self.artWorkNameTextField.text!)"
