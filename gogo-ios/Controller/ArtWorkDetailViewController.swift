...
class ArtWorkDetailViewController: BaseViewController {
  ...
  @IBOutlet weak var shareButton: UIBarButtonItem! {
    didSet {
      shareButton.setTitleTextAttributes([NSAttributedStringKey.font : UIFont.icomoon(ofSize: 20) ?? UIFont.systemFont(ofSize: 20),
  ...
  func basicData() {
    ...
    let artType = MainManager.sharedInstance.currentArtType
    let titleString = self.title!.replace(target: " ", withString:"_").lowercased()
    
    self.QRCodeStrings.append("http://gogo.tattoo/\(link)/\(artType.rawValue)/\(titleString)")
    self.QRCodeStrings.append("https://gogotattoo.github.io/\(link)/\(artType.rawValue)/\(titleString)")
...
//    }
  ...
  @IBAction func shareAction(_ sender: Any, event: UIEvent) {
    ...
    let arrowMenuViewModel0: DPArrowMenuViewModel = DPArrowMenuViewModel(title: "All Pictures",
                                                                         imageName: nil, image: UIImage(named: "place_holder"))
    ...
    DPArrowMenu.showForSender(sender: view, with: dataSource, done: {
      ...
      if selectedIndex == 1 { // TODO: optimize
        ...
        Manager.shared.loadImage(with: Request(url: URL(string: imageURLString)!)) {
          ...
          ShareManager.sharedInstance.shareTimelineAction(title: titleString,
                                                          description: "this is awesome",
                                                          image: retimage,
                                                          url: "https://gogotattoo.github.io/\(link)/\(artType.rawValue)/\(titleString)",
            sender: self)
        }
      }
      if selectedIndex == 2 {
