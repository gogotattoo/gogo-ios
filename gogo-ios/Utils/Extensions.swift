...
extension String {
  ...
  /// Generate utf-8 at runtime, for \u{} syntax is to be compiled
  ///
  /// - Parameter icon: hex string of iconfont
  /// - Returns: utf-8
  var convertHexToIconfont: String {
    let scanner = Scanner(string: self)
    var value: UInt64 = 0
    var retString = ""
    if scanner.scanHexInt64(&value) {
      if let temp = UnicodeScalar(UInt32(value)) {
        retString = String(Character(temp))
      }
    }
    return retString
  }
  
  /// Syntactic sugar for localization
  ...
  func substring(to index: Int) -> String {
    ...
    let offsetIndex = self.characters.index(self.startIndex, offsetBy: index)
  ...
  convenience init?(text: Iconfont, fontSize: CGFloat,
    ...
    if __CGSizeEqualToSize(imageSize, CGSize.zero) {
      imageRect = CGRect(origin: CGPoint.zero,
                         size: text.rawValue.size(withAttributes: [NSAttributedStringKey.font: iconfont]))
    }
    ...
    text.rawValue.draw(in: imageRect, withAttributes: [NSAttributedStringKey.font : iconfont,
                                                       NSAttributedStringKey.paragraphStyle: paragraphStyle,
                                                       NSAttributedStringKey.foregroundColor: imageColor])
