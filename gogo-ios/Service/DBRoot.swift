...
final public class DBRoot {
  ...
  func addTask(_ block: @escaping ClosureType) {
    ...
    while true {
      if self.reamlTaskRunloop == nil {
        debugPrint("waiting run loop ready!")
      } else {
        break
      }
    }
  ...
  /// Artist
  func insertArtists(_ artistViewModels: [ArtistViewModel],
  ...
  func selectArtists(_ completion: @escaping (_ result: [ArtistViewModel]?, _ error: NSError?)->()?) {
    let block = {
      autoreleasepool {
        do {
          var retArray: [ArtistViewModel] = []
          let realm = try Realm()
          realm.beginWrite()
  ...
  /// ArtWork
  func insertArtWorks(_ artWorkViewModels: [Any], artType: ArtType, artistLink: String,
                      completion: @escaping (_ result: Bool, _ error: NSError?)->()?) {
    let block = {
      autoreleasepool {
        do {
  ...
  func selectArtWorks(artType: ArtType, artistLink: String,
        ...
        do {
          ...
          for artWork in artWorks {
            ...
            case .dreadlocks:
              ...
              if dreadlocksViewModel.artistLink == artistLink && dreadlocksViewModel.artType == artType {
